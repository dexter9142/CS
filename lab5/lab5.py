...
def check():
    ...
    procent = int((len(succ)/(len(succ)+0.00001 + len(failed)))*100)
...
def restore():
        ...
        for i in range(len(failed)):
            struct = failed[i][0]
            query = 'reg add ' + struct['reg_key'] + ' /v ' + struct['reg_item'] + ' /d ' + failed[i][1] + ' /f'
            print('Query:', query)
...
def backup():
        f=open('backup.txt','w')
        backupString=json.dumps(failed)
...
def on_select_failed(evt):
    ...
    failed_selected=[]
    for i in actual:
        failed_selected.append(failed[i])
    localarr2=[]
    for i in actual:
